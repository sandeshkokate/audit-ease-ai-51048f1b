...
const NAV_ITEMS = [
  { title: 'Dashboard', url: '/platform-admin/dashboard', icon: BarChart3 },
  { title: 'Tenants', url: '/platform-admin/tenants', icon: Building2 },
  { title: 'Users', url: '/platform-admin/users', icon: Users },
  { title: 'Reports', url: '/platform-admin/reports', icon: FileBarChart },
  { title: 'Settings', url: '/platform-admin/settings', icon: Settings },
  { title: 'Activity Logs', url: '/platform-admin/activity-logs', icon: ScrollText },
  { title: 'Feature Flags', url: '/platform-admin/feature-flags', icon: ToggleLeft },
];
...
export default function PlatformAdminLayout() {
  ...
  return (
    <div className="flex min-h-screen w-full bg-background">
      <aside
        className={cn(
          'hidden md:flex flex-col sidebar-rich border-r border-sidebar-border transition-all duration-300 shrink-0',
          collapsed ? 'w-16' : 'w-60'
        )}
      >
        {sidebarContent}
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <aside className="relative flex h-full w-64 flex-col sidebar-rich shadow-elevated">
            {sidebarContent}
          </aside>
        </div>
      )}
