...
export default function ViewerLayout() {
  ...
  const sidebarContent = (
    <>
      <div className={cn('flex items-center gap-2.5 border-b border-sidebar-border px-4 h-16', collapsed && 'justify-center px-2')}>
        ...
        {!collapsed && <span className="text-lg font-bold text-sidebar-foreground whitespace-nowrap">Audit<span className="text-sidebar-primary">Ease</span></span>}
      </div>
  ...
  return (
    <div className="flex min-h-screen w-full bg-background">
      <aside className={cn('hidden md:flex flex-col sidebar-dark border-r border-sidebar-border transition-all duration-300 shrink-0', collapsed ? 'w-16' : 'w-60')}>{sidebarContent}</aside>
      {mobileOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          <div className="absolute inset-0 bg-foreground/30 backdrop-blur-sm" onClick={() => setMobileOpen(false)} />
          <aside className="relative flex h-full w-64 flex-col sidebar-dark shadow-2xl">{sidebarContent}</aside>
        </div>
      )}
