...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- Set defaults BEFORE the rate card lookup
      ...
      -- CHECK 2: Zone discrepancy — resolve using pincode_circle_map + zone_circle_matrix
      IF v_origin_pincode IS NOT NULL AND v_dest_pincode IS NOT NULL AND v_charged_zone != '' THEN
        -- First try existing zone_mapping
        ...
        -- If not found, resolve via circle map + matrix
            ...
            v_disc_reasons := v_disc_reasons || jsonb_build_array(
              jsonb_build_object(
                ...
                'detail', format('Charged zone %s vs expected zone %s',
                  v_charged_zone, v_resolved_zone)
              )
            );
          END IF;
        END IF;
      ...
      -- CHECK 3: RTO overcharge (no longer gated on rate_structure)
  ...
  RETURN jsonb_build_object(
    ...
    'message', format('Processed %s shipments, found %s discrepancies, %s failed, %s duplicates skipped',
      v_processed, v_discrepancies, v_failed, v_duplicates)
  );
