  ...
  LOOP
    BEGIN
      ...
      IF v_has_weight_disc OR v_has_zone_disc OR v_has_rto_overcharge THEN
        -- Start with billed amount as baseline
        v_expected_amount := v_billed_amount;

        -- Weight discrepancy: scale down proportionally
        ...
        -- Zone discrepancy without rate card: estimate 10-15% overcharge as conservative default
        IF v_has_zone_disc AND NOT v_has_weight_disc THEN
          -- If we have no rate card to calculate exact zone price difference,
          -- use a conservative 10% estimate for zone mismatch overcharge
        ...
        ELSE
          -- Reset flags if no meaningful financial impact
          v_has_weight_disc := FALSE;
          v_has_zone_disc := FALSE;
          v_has_rto_overcharge := FALSE;
          v_disc_reasons := '[]'::JSONB;
